...
char kOdomFrame[] = "odom";
char kBaseFrame[] = "base_footprint";
}
...
class WiFiHardware {
 public:
  ...
  void setConnection(const IPAddress& ip, uint16_t port) {
    server_ip_ = ip;
    server_port_ = port;
  }
...
void loop() {
  ...
  if (millis() - last_odom_publish_ms >= 50) {
    publishOdom();
    last_odom_publish_ms = millis();
  }
